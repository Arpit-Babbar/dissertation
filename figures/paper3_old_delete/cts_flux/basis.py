...
# points = gl, gll
# degree = 1,2,3,4,5
def fr_operators(scheme,points,degree,dgl,dgr, gl, gr):
    k = degree
    nd = k + 1
    if points == 'gl':
        # k+1 point gauss rule, integrates exactly upto degree 2*k+1
        xg, wg   = np.polynomial.legendre.leggauss(nd)
        xgg, wgg = np.polynomial.legendre.leggauss(nd+1)
    elif points == 'gll':
        if scheme == 'dfr':
            print('Cannot use GLL with DFR')
            exit()
        xg, wg   = lglnodes(nd-1)
        xgg, wgg = lglnodes(nd) # Higher degree to get continuous flux
    elif points == 'cheb1':
        # https://en.wikipedia.org/wiki/Chebyshev–Gauss_quadrature
        theta = (2.0*arange(nd,0,-1) - 1.0) * np.pi / (2.0*nd)
        xg = np.cos(theta)
        wg = (np.pi/nd) * np.sin(theta) # TODO
    elif points == 'cheb2':
        # https://en.wikipedia.org/wiki/Chebyshev–Gauss_quadrature
        theta = arange(nd,0,-1) * np.pi / (nd + 1)
        xg = np.cos(theta)
        wg = (np.pi/(nd + 1)) * np.sin(theta) # TODO
    else:
        print('Unknown points = ',points)
        exit()

    # Transform to [0,1]
    xg, xgg = 0.5*(xg + 1.0), 0.5*(xgg + 1.0)
    wg, wgg = 0.5*wg, 0.5*wgg

    print("sum weights = ", np.sum(wg))
    #assert(np.abs(np.sum(wg)-1.0) < 1.0e-14)
    ...
    if scheme == 'dfr':
        ...
        Gl, Gr = np.zeros(nd+1), np.zeros(nd+1)
        for i in range(nd):
            bl[i] = 2.0 * dgl(k, 2*xg[i]-1)
            br[i] = 2.0 * dgr(k, 2*xg[i]-1)
            Gl[i] = gl(k, 2.0*xgg[i]-1.0)
            Gr[i] = gr(k, 2.0*xgg[i]-1.0)
        D1 = Dm - np.outer(bl, Vl) - np.outer(br, Vr)
    else:
        print('Unknown scheme = ',scheme)
        exit()

    return xg,wg,xgg,wgg,Vl,Vr,bl,br,Dm,D1,Gl,Gr
