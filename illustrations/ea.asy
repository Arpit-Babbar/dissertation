...
pen p_big = linewidth(5.0)+blue;
draw( (0.,0.0)--(0.0,lower_height), pd, Arrow );
draw( (0.,upper_height)--(0.0,height), pd );
draw( (4.0*dx,0)--(4.0*dx,lower_height), pd, Arrow );
draw( (4.0*dx,upper_height)--(4.0*dx,height), pd );
...
dot((dx,0.0), p_big);
dot((2.0*dx,0.0), p_big);
dot((3.0*dx,0.0), p_big);

real eps = 0.1;
label("${\color{blue}F_{e-\frac{1}{2}}^+}$", (0.0,0.5*height));
label("${\color{blue}F_{e+\frac{1}{2}}^-}$", (4.0*dx,0.5*height));
